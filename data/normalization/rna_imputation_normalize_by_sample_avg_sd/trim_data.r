...
######## impute proteome using rna #############
## recenter
ref=cbind(apply(proteome_new,1,mean,na.rm=T),apply(proteome_new,1,sd,na.rm=T))
ind=!is.na(ref[,2])
rna_recentered=rna_new

d1=sum(ind);d2=dim(rna_new)[2]
mat_avg1=matrix(rep(ref[ind,1],times=d2),nrow=d1)
mat_sd1=matrix(rep(ref[ind,2],times=d2),nrow=d1)
mat_avg2=matrix(rep(apply(rna_new[ind,],1,mean,na.rm=T),times=d2),nrow=d1)
mat_sd2=matrix(rep(apply(rna_new[ind,],1,sd,na.rm=T),times=d2),nrow=d1)

rna_recentered[ind,] = (rna_new[ind,] - mat_avg2) / mat_sd2 * mat_sd1 + mat_avg1

## fill
tmp=proteome_new[,colnames(rna_new)]
tmp[is.na(tmp)]=rna_recentered[is.na(tmp)]
proteome_new[,colnames(rna_new)]=tmp
################################################

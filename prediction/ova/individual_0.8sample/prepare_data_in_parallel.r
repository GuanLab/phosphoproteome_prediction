...
feature=as.matrix(read.delim(paste0(path1,"raw_ova_proteome.txt.scaled.avg_imputation.subset1.0"),check.names=F,row.names=1))
train=as.matrix(read.delim(paste0(path2,"train_ova_phospho_",i,".txt"),check.names=F,row.names=1))
...
test=as.matrix(read.delim(paste0(path2,"test_ova_phospho_",i,".txt"),check.names=F,row.names=1))

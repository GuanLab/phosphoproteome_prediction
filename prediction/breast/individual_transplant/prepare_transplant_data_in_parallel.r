...
# training data
ot=as.matrix(read.delim(paste0(path0,"ova_phospho_old_version.txt"),check.names=F,row.names=1))
bt=as.matrix(read.delim(paste0(path2,"train_breast_phospho_",i,".txt"),check.names=F,row.names=1))
ind_target=intersect(rownames(ot),rownames(bt)) # common target gene
train=cbind(ot[ind_target,],bt[ind_target,])

# test
test_all=as.matrix(read.delim(paste0(path2,"test_breast_phospho_",i,".txt"),check.names=F,row.names=1))

...
mat_num=as.matrix(read.delim("breast_num_sample.txt",row.names=1,header=F))

for (j in 1:5){
	input=as.matrix(read.delim(paste0(path1,"prediction_breast_phospho_",j,".txt"),check.names=F,row.names=1))
	output=input
	...
	write.table(tmp,file=paste0(path2,"prediction_bb_phospho_",j,".txt"),quote=F,sep="\t",row.names=F)
}

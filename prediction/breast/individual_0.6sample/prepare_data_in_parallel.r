...
feature=as.matrix(read.delim(paste0(path1,"raw_breast_proteome.txt.scaled.avg_imputation.subset1.0"),check.names=F,row.names=1))
train=as.matrix(read.delim(paste0(path2,"train_breast_phospho_",i,".txt"),check.names=F,row.names=1))

############# sub-sample training set ##########
set.seed(i)
ratio=0.6

nt=round(dim(train)[2]*ratio,0)
train=train[,sort(sample(dim(train)[2],nt))]
################################################

test=as.matrix(read.delim(paste0(path2,"test_breast_phospho_",i,".txt"),check.names=F,row.names=1))

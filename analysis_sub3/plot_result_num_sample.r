...
for(i in 1:5){
    tmp=read.delim(paste0("../sub3/prediction/breast/individual_0.4sample/cor_nrmse_b",i,".txt"),header=F,row.names=1)
    cor1[,1,i]=tmp[,1]
    rmse1[,1,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/breast/individual_0.6sample/cor_nrmse_b",i,".txt"),header=F,row.names=1)
    cor1[,2,i]=tmp[,1]
    rmse1[,2,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/breast/individual_0.8sample/cor_nrmse_b",i,".txt"),header=F,row.names=1)
    cor1[,3,i]=tmp[,1]
    rmse1[,3,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/breast/individual/cor_nrmse_b",i,".txt"),header=F,row.names=1)
    cor1[,4,i]=tmp[,1]
    rmse1[,4,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/breast/individual_transplant/cor_nrmse_b",i,".txt"),header=F,row.names=1)
    cor1[,5,i]=tmp[,1]
    rmse1[,5,i]=tmp[,2]
}
for(i in 1:5){
    tmp=read.delim(paste0("../sub3/prediction/ova/individual_0.4sample/cor_nrmse_o",i,".txt"),header=F,row.names=1)
    cor2[,1,i]=tmp[,1]
    rmse2[,1,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/ova/individual_0.6sample/cor_nrmse_o",i,".txt"),header=F,row.names=1)
    cor2[,2,i]=tmp[,1]
    rmse2[,2,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/ova/individual_0.8sample/cor_nrmse_o",i,".txt"),header=F,row.names=1)
    cor2[,3,i]=tmp[,1]
    rmse2[,3,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/ova/individual/cor_nrmse_o",i,".txt"),header=F,row.names=1)
    cor2[,4,i]=tmp[,1]
    rmse2[,4,i]=tmp[,2]
    tmp=read.delim(paste0("../sub3/prediction/ova/individual_transplant/cor_nrmse_o",i,".txt"),header=F,row.names=1)
    cor2[,5,i]=tmp[,1]
    rmse2[,5,i]=tmp[,2]
}

name2=c("28(40%)","41(60%)","55(80%)","69(100%)","174(two tissues)")
...
p1 = ggplot(dat.m, aes(model, correlation, fill=model)) + 
    ...
    labs(x="number of samples", y="Pearson's correlation",title="breast") +
    theme_light() +
...
p2 = ggplot(dat.m, aes(model, correlation, fill=model)) + 
    geom_violin(width=0.5, colour="grey50",draw_quantiles = 0.5) +
    scale_fill_manual(values=tmp_col) +
    labs(x="number of samples", y="NRMSE",title="breast") +
    theme_light() +
    theme(plot.title = element_text(hjust = 0.5),legend.position="none") +
    #ylim(0.1,0.35) +
    theme(axis.line.x = element_line(color="black", size = 0.5))+
    ...
    theme(axis.text.x = element_text(colour="black",size=12))+
    theme(axis.text.y = element_text(colour="black",size=12))+
    annotate("text", x = seq(1.5,4.5,1), y = 0.28, label=paste0("p=",format(pval,digits=3)),size=4)+
    annotate("text", x = 1:5, y = 0.3, label=paste0(format(dat[1,],digits=3)),size=5)
#    geom_rect(mapping=aes(xmin=2.5,xmax=3.5,ymin=0.25,ymax=0.45,fill=NA),color=my_palette["yellow"])
p2
...
p3 = ggplot(dat.m, aes(model, correlation, fill=model)) + 
    geom_violin(width=0.5, colour="grey50",draw_quantiles = 0.5) +
    scale_fill_manual(values=tmp_col) +
    labs(x="number of samples", y="Pearson's correlation",title="ovary") +
    theme_light() +
    theme(plot.title = element_text(hjust = 0.5),legend.position="none") +
    #ylim(0.1,0.35) +
    theme(axis.line.x = element_line(color="black", size = 0.5))+
    ...
    theme(axis.text.x = element_text(colour="black",size=12))+
    theme(axis.text.y = element_text(colour="black",size=12))+
    annotate("text", x = seq(1.5,4.5,1), y = 0.3, label=paste0("p=",format(pval,digits=3)),size=4)+
    annotate("text", x = 1:5, y = 0.404, label=paste0(format(dat[1,],digits=3)),size=5)
#    geom_rect(mapping=aes(xmin=2.5,xmax=3.5,ymin=0.25,ymax=0.45,fill=NA),color=my_palette["yellow"])
p3
...
p4 = ggplot(dat.m, aes(model, correlation, fill=model)) + 
    geom_violin(width=0.5, colour="grey50",draw_quantiles = 0.5) +
    scale_fill_manual(values=tmp_col) +
    labs(x="number of samples", y="NRMSE",title="ovary") +
    theme_light() +
    theme(plot.title = element_text(hjust = 0.5),legend.position="none") +
    #ylim(0.1,0.35) +
    theme(axis.line.x = element_line(color="black", size = 0.5))+
    ...
    annotate("text", x = 1:5, y = 0.39, label=paste0(format(dat[1,],digits=3)),size=5)
#    geom_rect(mapping=aes(xmin=2.5,xmax=3.5,ymin=0.25,ymax=0.45,fill=NA),color=my_palette["yellow"])
p4


pdf(file="figure/result_num_sample.pdf",width=10,height=6,useDingbats=F)
...
write.csv(tbl,file="table/result_num_sample_cor1.csv")

tbl=apply(cor2,3:2,mean,na.rm=T)
rownames(tbl)=name1; colnames(tbl)=name2
write.csv(tbl,file="table/result_num_sample_cor2.csv")

tbl=apply(rmse1,3:2,mean,na.rm=T)
rownames(tbl)=name1; colnames(tbl)=name2
write.csv(tbl,file="table/result_num_sample_rmse1.csv")

tbl=apply(rmse2,3:2,mean,na.rm=T)
rownames(tbl)=name1; colnames(tbl)=name2
write.csv(tbl,file="table/result_num_sample_rmse2.csv")
